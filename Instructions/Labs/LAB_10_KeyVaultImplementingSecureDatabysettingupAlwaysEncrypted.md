...
1. En el panel **Editar plantilla**, haga clic en **Cargar archivo**, busque el archivo **\\Allfiles\\Labs\\10\\az-500-10_azuredeploy.json** y haga clic en **Abrir**.

1. En el panel **Editar plantilla**, haga clic en **Guardar**.

1. En el panel **Implementación personalizada**, en **Ámbito de la implementación**, asegúrese de que las siguientes opciones están configuradas (deje las demás con los valores predeterminados):
...
1. Haga clic en el botón **Revisar y crear** y confirme la implementación haciendo clic en el botón **Crear**. 
...
1. En el panel **Grupos de recursos**, en la lista del grupos de recursos, haga clic en la entrada **AZ500LAB10** (o el otro nombre que eligió anteriormente para el grupo de recursos).

1. En el panel Grupo de recursos, haga clic en la entrada que representa el Key Vault recién creado. 

1. En el panel Key Vault, en la sección **Configuración**, haga clic en **Directivas de acceso** y, a continuación, haga clic en **+ Agregar directiva de acceso**.

1. En el panel **Agregar directiva de acceso**, configure las opciones siguientes (deje todas las demás con los valores predeterminados): 

    |Configuración|Value|
    |----|----|
    |Configurar a partir de una plantilla (opcional)|**Administración de claves, secretos y certificados**|
    |Permisos de claves|haga clic en **Seleccionar todo**, lo que da como resultado **16 permisos seleccionados**|
...
1. De nuevo en el panel **Agregar directiva de acceso**, haga clic en **Agregar** para agregar la directiva de acceso y, de nuevo en el panel directivas de acceso del Key Vault, haga clic en **Guardar** en los cambios. 
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para agregar una clave protegida por software al Key Vault: 
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para comprobar que se creó la clave:

    ```powershell
    Get-AZKeyVaultKey -VaultName $kv.VaultName
    ```

1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para mostrar el identificador de la clave:

    ```powershell
    $key.key.kid
    ```

1. Minimice el panel de Cloud Shell. 

1. De nuevo en Azure Portal, en el panel de Key Vault, en la sección **Configuración**, haga clic en **Claves**.

1. En la lista de claves, haga clic en la entrada **MyLabKey** y, a continuación, en el panel **MyLabKey**, haga clic en la entrada que representa la versión actual de la clave.
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para crear una variable con un valor de cadena segura:

    ```powershell
    $secretvalue = ConvertTo-SecureString 'Pa55w.rd1234' -AsPlainText -Force
    ```

1.  En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para agregar el secreto al almacén:
...
1.  En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para comprobar que se creó el secreto.
...
1. En Azure Portal, vaya al panel de Key Vault y, en la sección **Configuración**, haga clic en **Secretos**.

1. En la lista de secretos, haga clic en la entrada **SQLPassword** y, a continuación, en el panel **SQLPassword**, haga clic en la entrada que representa la versión actual del secreto.
...
1. Abra el panel **Registros de la aplicación** y haga clic en **+ Nuevo registro**. 

1. En el panel **Registrar una aplicación**, configure las opciones siguientes (deje todas las demás con los valores predeterminados):
...
1. En el panel **Registrar una aplicación**, haga clic en **Registrar**. 

    >**Nota**: Una vez completado el registro, el explorador le redirigirá automáticamente al panel **sqlApp**. 

1. En el panel **sqlApp**, identifique el valor de **Id. de aplicación (cliente)** . 

    >**Nota**: Registre este valor. Lo necesitará en la próxima tarea.

1. En el panel **sqlApp**, en la sección **Administrar**, haga clic en **Certificados y secretos**.

1. En el panel **sqlApp | Certificados y secretos**, en la sección **Secretos de cliente**, haga clic en **+ Nuevo secreto de cliente**

1. En el panel **Agregar un secreto de cliente**, especifique la siguiente configuración:
...
1. Haga clic en **Agregar** para actualizar las credenciales de la aplicación.

1. En el panel **sqlApp | Certificados y secretos**, identifique el valor de **Key1**.
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para crear una variable que almacene el **id. de aplicación (cliente)** que registró en la tarea anterior (reemplace el marcador de posición `<Azure_AD_Application_ID>` por el valor del **id. de aplicación (cliente)** ):
   
    ```powershell
    $applicationId = '<Azure_AD_Application_ID>'
    ```
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para crear una variable que almacene el nombre de Key Vault.
    ```
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para conceder permisos en Key Vault a la aplicación que registró en la tarea anterior:
...
1. En la lista de bases de datos SQL, haga clic en la entrada **medicina(<randomsqlservername>)** .
...
1. En el panel base de datos SQL, en la sección **Configuración**, haga clic en **Cadenas de conexión**. 

    >**Nota**: La interfaz incluye cadenas de conexión para ADO.NET, JDBC, ODBC, PHP y Go. 
   
1. Copie la **cadena de conexión para ADO.NET**. Lo necesitará más adelante.
...
>**Nota**: Antes de continuar con esta tarea, asegúrese de que la implementación que inició en el primer ejercicio se ha completado correctamente. Para validarlo, puede ir al panel del grupo de recursos de Azure "Az500Lab10" (o el otro nombre que eligió) y seleccionar **Implementaciones** en el panel Configuración.  
...
1. En la lista de máquinas virtuales que se muestra, seleccione la entrada **az500-10-vm1**. En el panel **az500-10-vm1**, en el panel **Essentials**, tome nota de la **dirección IP pública**. Utilizará esta dirección más adelante. 
...
1. En Azure Portal, vaya al panel de la base de datos SQL **Medicina**, en la sección **Essentials**, identifique el nombre del servidor y, a continuación, en la barra de herramientas, haga clic en **Establecer el firewall del servidor**.  

    >**Nota**: Registre el nombre del servidor. Necesitará el nombre del servidor más adelante en esta tarea.

1. En el panel **Configuración del firewall**. desplácese hacia abajo hasta **Nombre de regla** y especifique la siguiente configuración: 
...
1. Haga clic en **Guardar** y **Aceptar** para guardar el cambio y cerrar el panel de confirmación. 
...
1. Vuelva al panel **az500-10-vm1**, haga clic en **Introducción** y,a continuación, haga clic en **Conectar** y, en el menú desplegable, haga clic en **RDP**. 

1. Haga clic en **Descargar archivo RDP** y úselo para conectarse a la máquina virtual de Azure **az500-10-vm1** a través de Escritorio remoto. Cuando se le pida que se autentique, proporcione las credenciales siguientes:
...
1. Haga clic en **Inicio**, en el menú **Inicio**, expanda la carpeta **Microsoft SQL Server Tools 18** y haga clic en el elemento de menú **Microsoft SQL Server Management Studio**.

1. En el cuadro de diálogo **Conectar con el servidor**, aplique la configuración siguiente: 
...
1. En el cuadro de diálogo **Conectar con el servidor**, haga clic en **Conectar**.

1. En la consola de **SQL Server Management Studio**, en el panel **Explorador de objetos**, expanda la carpeta **Bases de datos**.

1. En el panel **Explorador de objetos**, haga clic con el botón derecho en la base de datos **medicina** y haga clic en **Nueva consulta**.

1. Pegue el código siguiente en la ventana de consulta y haga clic en **Ejecutar**. Esto creará una tabla de **Pacientes**.
     ...
     ```
1. Una vez creada correctamente la tabla, en el panel **Explorador de objetos**, expanda el nodo de la base de datos **medicina**, el nodo de **tablas** y haga clic con el botón derecho en el nodo **dbo.Patients** y haga clic en **Cifrar columnas**. 

    >**Nota**: Esto iniciará el asistente para **Always Encrypted**.

1. En la página **Introducción**, haga clic en **Siguiente**.

1. En la página **Selección de columna**, seleccione las columnas **SSN** y **Fecha de nacimiento**, establezca el **Tipo de cifrado** de la columna **SSN** en **Determinista** y el de la columna **Fecha de nacimiento** en **Aleatorio** y haga clic en **Siguiente**.

1. En la página **Configuración de clave maestra**, seleccione **Azure Key Vault**, haga clic en **Iniciar sesión**, cuando se le solicite, autentíquese con la misma cuenta de usuario que usó para aprovisionar la instancia de Azure Key Vault anteriormente en este laboratorio, asegúrese de que Key Vault aparezca en la lista desplegable **Seleccionar un Azure Key Vault** y haga clic en **Siguiente**.

1. En la página **Ejecutar configuración**, haga clic en **Siguiente**.
    
1. En la página **Resumen**, haga clic en **Finalizar** para continuar con el cifrado. Cuando se le solicite, vuelva a iniciar sesión con la misma cuenta de usuario que usó para aprovisionar la instancia de Azure Key Vault anteriormente en este laboratorio.

1. Una vez completado el proceso de cifrado, en la página **Resultados**, haga clic en **Cerrar**.

1. En la consola de **SQL Server Management Studio**, en el panel **Explorador de objetos**, en el nodo **medicina**, expanda los subnodos **Seguridad** y **Claves de Always Encrypted**. 
...
1. Cambie a la ventana que muestra el mensaje de bienvenida de Visual Studio 2019, haga clic en el botón **Iniciar sesión** y, cuando se le solicite, proporcione las credenciales que usó para autenticarse en la suscripción de Azure que usa en este laboratorio.

1. En la página **Comenzar**, haga clic en **Crear un proyecto nuevo**. 

1. En la lista de plantillas de proyecto, busque **Aplicación de consola (.NET Framework)** , en la lista de resultados, haga clic en **Aplicación de consola (.NET Framework)** para **C#** y haga clic en **Siguiente**.

1. En la página **Configurar el nuevo proyecto**, especifique las siguientes opciones (deje las demás opciones con sus valores predeterminados) y, a continuación, haga clic en **Crear**:
...
1. En la consola de Visual Studio, haga clic en el menú **Herramientas**, en el menú desplegable, haga clic en **Administrador de paquetes NuGet** y, en el menú en cascada, haga clic en **Consola del administrador de paquetes**.

1. En el panel **Consola del administrador de paquetes**, ejecute lo siguiente para instalar el primer paquete **NuGet** necesario:

    ```powershell
    Install-Package Microsoft.SqlServer.Management.AlwaysEncrypted.AzureKeyVaultProvider
    ```

1. En el panel **Consola del administrador de paquetes**, ejecute lo siguiente para instalar el segundo paquete **NuGet** necesario:

    ```powershell
    Install-Package Microsoft.IdentityModel.Clients.ActiveDirectory
    ```
    
1. Vaya a **\\Allfiles\\Labs\\10\\program.cs**, ábralo en el Bloc de notas y copie su contenido en el Portapapeles.

1. Cambie a la consola de Visual Studio, en la ventana **Explorador de soluciones**, haga clic en **Program.cs** y reemplace su contenido por el código que copió en el Portapapeles.

1. En la ventana de Visual Studio, en el panel **Program.cs** en la línea 15, reemplace el marcador de posición `<connection string noted earlier>` por la cadena de conexión **ADO.NET** de la base de datos de Azure SQL que registró anteriormente en el laboratorio. En la cadena de conexión, reemplace el marcador de posición `{your_password}` por `Pa55w.rd1234`.

1. En la ventana de Visual Studio, en el panel **Program.cs** en la línea 16, reemplace el marcador de posición `<client id noted earlier>` por el valor del **id. de aplicación (cliente)** de la aplicación registrada que registró anteriormente en el laboratorio. 

1. En la ventana de Visual Studio, en el panel **Program.cs** en la línea 17, reemplace el marcador de posición `<key value noted earlier>` por el valor de **Key1** de la aplicación registrada que registró anteriormente en el laboratorio. 

1. En la consola de Visual Studio, haga clic en el botón **Iniciar** para iniciar la compilación de la aplicación de consola e iniciarla.

1. La aplicación iniciará una ventana del símbolo del sistema. Cuando se le solicite la contraseña, escriba **Pa55w.rd1234** para conectarse a Azure SQL Database. 

1. Deje la aplicación de consola en ejecución y cambie a la consola de **SQL Management Studio**. 

1. En el panel **Explorador de objetos**, haga clic con el botón derecho en la base de datos de medicina y, abriendo el menú con el botón derecho, haga clic en **Nueva consulta**.

1. En la ventana de consulta, ejecute la consulta siguiente para comprobar que los datos cargados en la base de datos desde la aplicación de consola están cifrados.

    ```sql
    SELECT FirstName, LastName, SSN, BirthDate FROM Patients;
    ```

1. Vuelva a la aplicación de consola, donde se le pedirá que escriba un SSN válido. Esto consultará los datos en la columna cifrada. En el símbolo del sistema, escriba lo siguiente y presione la tecla Entrar:
...
1. Para finalizar la aplicación de consola, presione la tecla Entrar
